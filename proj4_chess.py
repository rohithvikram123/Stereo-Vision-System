...
image_1 = cv.imread("chessim0.png")
image_2 = cv.imread("chessim1.png")
image1 = image_1.copy()
image2 = image_2.copy()
campoints1,campoints2 = feature_detector(image_1,image_2)
print(len(campoints1))
...
print("Fundemental Matrix")
print(Fmatrix)
Intrinsic=np.array([[1758.23,0,829.15],[0,1758.23,552.78],[0,0,1]])
...
print(H2)
left_epilines = cv.warpPerspective(image1,H1,[1920,1080])
right_epilines = cv.warpPerspective(image2,H2,[1920,1080])
left_rectified = cv.warpPerspective(image_1,H1,[1920,1080])
right_rectified = cv.warpPerspective(image_2,H2,[1920,1080])
...
for i in np.arange(3, height1-3):
    for j in np.arange(3, width1-3):
        ...
        if int(disp) == 0:
            disp = 0.0000000001
            Depth_eval[j][i] = 1758.23*97.99/disp
        else:
            Depth_eval[j][i] = 1758.23*97.99/disp
...
disparity_error.T, None, 65, 197, cv.NORM_MINMAX, cv.CV_8U)
...
cv.imshow("disparitycolor_map.png", disparity_colormap)
depth_normalized = cv.normalize(
    Depth_eval.T, None, 65, 197, cv.NORM_MINMAX, cv.CV_8U)
depth_colormap = cv.applyColorMap(depth_normalized, cv.COLORMAP_JET)
cv.imshow("depth_grayscale.png", depth_normalized)
cv.imshow("depthcolor_map.png", depth_colormap)
