...
image_1 = cv.imread("artroomim0.png")
image_2 = cv.imread("artroomim1.png")
image1 = image_1.copy()
image2 = image_2.copy()
...
print("Fundemental Matrix")
print(Fmatrix)
Intrinsic=np.array([[1733.74,0,792.27],[0,1733.74,541.89],[0,0,1]])
...
for i in np.arange(3, height1-3):
    for j in np.arange(3, width1-3):
        ...
        if int(disp) == 0:
            disp = 0.0000000001
            Depth_eval[j][i] = 1733.74*536.62/disp
        else:
            Depth_eval[j][i] = 1733.74*536.62/disp
...
disparity_normalized = cv.normalize(
    disparity_error.T, None, 55, 142, cv.NORM_MINMAX, cv.CV_8U)
...
depth_normalized = cv.normalize(
    Depth_eval.T, None, 55, 142, cv.NORM_MINMAX, cv.CV_8U)
